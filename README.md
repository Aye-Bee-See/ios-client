...
# The API's answers of 22 September (pull request #117) on the same end-to-end throwaway.
cd ABCMailboxKit && ABC_LIVE_THROWAWAY=http://localhost:3199 swift test --filter testThrowawayServerClientFeedback
...
## The API's answers of 22 September (pull request #117)

- **No automatic fallback** to the password: see "An account from before" above.
- **`condition` on coded refusals.** A dead claim token or invite code, and a refused deletion, are worded by the code the API sends beside the sentence, never by matching words in it. Errors without a finer code have none, and keep the server's sentence.
- **`returnNote` on the letter.** A conversation no longer reads every returned letter by itself for the group's note; against an older API, which sends none, it still does.
- **Held counts on conversation rows.** A conversation with a held letter says so in the list: "A letter is waiting for you to decide" when a hold waits on the writer (`choose_relay`, `reseal_needed`), "N letters on hold" when it waits on the group (`prisoner_free`).
- Keyless split accounts on a server-mode API were already what this app sends.
...
*Where it goes.* Sign-in is the handshake, the derivation, then one request (`SessionRepository.signIn`). Claim, password change and recovery send `authScheme: "split"`, the auth key, and the key fields re-wrapped under the wrap key; deleting an account proves the auth key. A split account with no keys yet (made by an admin) gets them at first sign-in, wrapped under that sign-in's wrap key with the same salt. The phone remembers which usernames it has signed in to as split (`SchemeMemory`, in UserDefaults, per server address, never cleared by signing out or by deleting an account) and refuses to sign in to one of them as plain whatever the server says, with a sentence that says why. An older API (404 on the handshake) means plain everywhere; a handshake that answers with anything other than a complete `split` or an explicit `plain` is refused, so a malformed or tampered answer cannot be a way to be sent the password.

*An account from before.* With the flag on, the handshake answers "split" for every name so as to say nothing about any of them, and an account from before the scheme can only sign in with the password itself. The API decided (its brief, item 23, API pull request #117): every account is moved to split before the flag goes on, and a client never sends the password on its own after a refused auth key. So there is no automatic fallback. What remains is an explicit choice on the sign-in screen, "This is an account from before the app stopped sending passwords: sign in with the password itself. Only if a superadmin told you to.", which sends the password as it is, once, by the person's decision and never for a name this phone knows as split. The session remembers the way it was signed in, so a later proof of the password (a change, a deletion, an unlock) goes the same way, until a password change moves the account to split.
...
- **The #117 answers, against a real end-to-end API with the flag on (23 September 2026).** On a throwaway at the API's `main` (#118): the ordinary sign-in of `member1` and `user1`, accounts from before, was refused with one request each and no fallback; the explicit choice signed them in; `member1`'s password change, proven the remembered way, moved it to split and the next sign-in was one derivation; a returned letter's note came with the conversation and no history was read; a conversation with a held letter said `heldCount` 1, `prisoner_free`; the last key holder's deletion was refused with `condition: last_key_holder`. **Not seen on screen:** the older-account choice on the sign-in screen and the held notice on conversation rows (they compile).
