...
Written 19 September 2026 on a Mac whose Xcode had no iOS simulator runtime installed, so:

- **Verified.** All tests pass. Crypto interoperates with the API and libsodium.js in both directions. Against the live development servers: the directory, the offline download, a writer's threads, a group's queue and writers in server mode; and in end-to-end mode, a group member's key (wrapped by the Android app) opened with their password, the group key opened with that, and letters written by other clients decrypted. The app target compiles and links in Xcode's own build system, Debug and Release, with no warnings.
- **Not verified.** The app has never been launched. Nothing on screen has been seen, tapped, or read by VoiceOver; the asset catalog (icon, accent colour) has not been through `actool`, which needs a simulator runtime. Expect the ordinary first-run findings of a new SwiftUI app: spacing, a keyboard covering a field, a transition that looks wrong. Install a simulator (Xcode, Settings, Components) and walk through the "what to try by hand" lists in `../Android/docs/PLAN.md`; they apply unchanged.
